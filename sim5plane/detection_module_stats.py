...
data_path=parent_path+'/5minplanedata/npymats'
...
def image_loading(num,addFore):  
    ...
    for j in valindices:
        ...
        pssmth=np.load(data_path+'/ps_'+str(i)+'_'+str(rem)+'.npy')
        val_pssmth.append(np.array(pssmth))
        label=np.load(data_path+'/segps_'+str(i)+'_'+str(rem)+'.npy')
        val_label.append(np.array(label))
        if addFore:
            full_fore=np.load(data_path+'/full_fore_'+str(i)+'_'+str(rem)+'.npy')
            val_full_fore.append(np.array(full_fore))
            mfiltered_fore=np.load(data_path+'/mfiltered_fore_'+str(i)+'_'+str(rem)+'.npy')
    ...
    if addFore:
        val_full_fore=np.array(val_full_fore).reshape(3072*nmaps-lenj,size,size,1)
        val_predictions_fore=model.predict(val_full_fore)
        return (val_mfiltered,val_predictions,val_pssmth,val_full,val_label,val_full_fore,val_mfiltered_fore,val_predictions_fore)
    else:
        return (val_mfiltered,val_predictions,val_pssmth,val_full,val_label)
...
def total_comparator(val_mfiltered, val_predictions, val_ps, val_full, val_label):
    ...
    #above_detections,below_detections,undet./mected_above,undetected_below,real_objects,spur_objects)
    val_mfiltered=np.array(val_mfiltered).reshape(len(val_ps),150,150,1)
    threshold=[0.28,0.39] #CNN thresholds to be evaluated.
    mfil_threshold=[4.02,4.58] #MF thresholds to be evaluated (in sigmas)
    num=60 #number of fluxes to be evaluated
    fluxes=np.linspace(1,250,num=num) #Flux in Tps
    ...
    for thr_ind in range(len(mfil_threshold)):
        thr=mfil_threshold[thr_ind]
        plt.plot(fluxes*0.0009212, mfil_completeness[thr_ind,:], label='MF '+str(thr), color=colours[thr_ind])
        print("Spureous ratio, MF at threshold "+str(thr)+" sigma: "+str(mfil_spures[thr_ind]))
    plt.xlabel('S / Jy')
    plt.ylabel('Completeness')
    plt.legend()
    plt.savefig('fullcompl5minplanefore.svg',format='svg')
    plt.savefig('fullcompl5minplanefore.png',format='png', dpi=300)
    plt.show()
    plt.close()
...
#val_mfiltered,val_predictions,val_pssmth,val_full,val_label,val_full_fore, val_mfiltered_fore,val_predictions_fore=image_loading(3072, True)

results2=total_comparator(val_mfiltered_fore,val_predictions_fore,val_pssmth,val_full_fore,val_label)

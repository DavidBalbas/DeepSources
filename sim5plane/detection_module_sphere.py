...
def image_loading(i):  
    ...
    for rem in range(lenj):
        posmap=np.load(data_path+'/location_'+str(rem)+'.npy').astype(np.int32)
...
#        val_full[ind]=val_full[ind]+val_pssmth[ind+100]+val_pssmth[ind+200]+val_pssmth[ind+300]
    ...
    val_full_fore=np.array(val_full_fore).reshape(lenj,size,size,1)
    ...
    print('Images loaded')
    return(val_full, val_full_fore, val_predictions, val_predictions_fore, val_mfiltered, val_mfiltered_fore, val_pssmth, val_label, positions)
...
def plot_map_withsources(positionalmap,foregrounds):
    ...
    hp.mollview(foregrounds,cmap=plt.get_cmap('Blues'),max=1,min=-0.15,title='')#, title='PS location, t='+str(threshold)+r',$ S_{min}=$'+str(fluxjy)+' Jy')
    ...
    plt.savefig('pslocationsfore.svg', format='svg')
    plt.savefig('pslocationsfore.png', format='png', dpi=400)
...
#Small script to carry out the evaluation and comparison.
threshold=0.28 
fluxjy=0.080
flux=fluxjy/0.0009212
val_full, val_full_fore, val_predictions, val_predictions_fore, val_mfiltered, val_mfiltered_fore, val_pssmth, val_label, val_locations=image_loading(1)
positionalmap, above_detections, below_detections, undetected_below, undetected_above, real_objects, spur_objects=draw_globalmap_cnn(threshold, flux, val_full_fore, val_predictions_fore, val_pssmth, val_label, val_locations)
##can include _fore
positionalmap=positionalmap.astype(float)
##hp.mollview(positionalmap)
##Create a foreground-numbered map if desired
numberedmap=np.zeros(12*2048*2048)
numberedmap[0]=1

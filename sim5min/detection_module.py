...
def total_comparator(val_mfiltered, val_predictions, val_ps, val_full, val_label):
    ...
    mfil_threshold=[3.06,3.26,3.66] #MF thresholds to be evaluated (in sigmas)
    num=20 #number of fluxes to be evaluated
    fluxes=np.linspace(35,110,num=num) #Flux in Tps
    ...
    plt.legend()
    plt.savefig('fullcompl5min-zoom.svg',format='svg')
...
#val_mfiltered,val_predictions,val_pssmth,val_full,val_label=image_loading(3072)

#results2=total_comparator(val_mfiltered,val_predictions,val_pssmth,val_full,val_label)
